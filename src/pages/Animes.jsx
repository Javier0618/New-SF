...
export const Animes = () => {
  ...
  useEffect(() => {
    let result = animesList;

    if (searchTerm) {
      result = result.filter(
        (a) => matchesSearch(a.title || a.name, searchTerm) || matchesSearch(a.original_title || a.original_name, searchTerm)
      );
    }

    if (selectedGenre) {
      result = result.filter((a) =>
        a.genres?.some((g) => (typeof g === 'object' ? g.name : g) === selectedGenre)
      );
    }

    setFilteredAnimes(result);
  }, [searchTerm, selectedGenre, animesList]);

  const allGenres = Array.from(
    new Set(
      animesList.flatMap((a) =>
        (a.genres || []).map((g) => (typeof g === 'object' ? g.name : g))
      )
    )
  ).filter(Boolean);
    ...
    <div className="min-h-screen bg-brand-dark pt-24 pb-16 px-4 sm:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-brand-border/60 pb-6">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-white flex items-center gap-3">
            <Sparkles className="w-8 h-8 text-purple-500" /> Animes
          </h1>
          <p className="text-slate-400 text-sm mt-1">Explora las mejores series y películas de animación japonesa</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 text-slate-400 bg-brand-card/40 rounded-2xl border border-brand-border/40">
          ...
          <p className="text-lg font-medium text-slate-300">No se encontraron animes</p>
